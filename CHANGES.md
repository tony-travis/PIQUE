...
## SNP-level heterozygosity filter (`-H`)

A new option, `-H het_max`, has been added to `pique-input`. It removes SNPs where the
observed proportion of heterozygous individuals exceeds the supplied threshold. The value
must be a number between 0 and 1 (e.g. `-H 0.005` removes SNPs where more than 0.5% of
...
and that the MAF threshold is then applied to the already-filtered data. When `-H` is
...
or `'maf'`), so that the missingness and MAF filters can be called separately when `-H`
...
- `input_snphet` / `run_snphet` -- runs `pique-input` with `-H 0.005` and then EMMAX
- `input_snpzhet` / `run_snpzhet` -- runs both `-H 0.005` and `-z 0.999` together, to
...
`-H` filter. It will be fixed in a separate follow-up.
